...
\cite{paper1} used an Artificial Neural Network (ANN) to non-linearly calibrate PT100 sensor readings across $-50^\circ$C to $150^\circ$C. Achieved $>91\%$ accuracy improvement, reducing error from $\sim 1.3^\circ$C to $\sim 0.27^\circ$C.
  The implementation was entirely software-based (offline). The study \textbf{did not implement the complex ANN model on any microcontroller} (e.g., Arduino, STM32), leaving the real-time processing feasibility and computational overhead on embedded hardware unaddressed.
    % --- Paper 2: New Circuit Design (376796536) ---
 \cite{paper2}
  Designed and implemented a high-accuracy temperature measurement circuit using a Wheatstone bridge, an AD623 differential amplifier, and an STM32F4 microcontroller. Achieved a very high detection accuracy of $\mathbf{0.025^\circ}$C.
  High accuracy relies on expensive, specialized analog components and a high-performance microcontroller. Critically, there is a \textbf{lack of machine learning and comparative study} against contemporary digital calibration or ML methods.
\cite{paper3}
 Focused on the simulation and design of the measurement transducer. Used a DC balance Wheatstone bridge configuration and Multisim software to model the circuit, achieving high simulated accuracy (high $\mathbf{R^2}$ and low MSE) for a $10^\circ$C to $90^\circ$C range.
 The findings are primarily based on **software simulation** only. The study \textbf{did not implement or compare against Machine Learning} models, relying solely on theoretical circuit performance.
...
\cite{paper4}
 ...
 Focuses only on the dynamic modeling (time delay/response) of the sensor, not the static (steady-state) calibration or non-linearity correction, which is critical for overall accuracy across a wide range. It requires specialized dynamic testing equipment.
