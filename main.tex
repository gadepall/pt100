...
\begin{equation}
    R(t) = R_0 (1 + A t + B t^2)
\end{equation}
for temperatures $t > 0\degree$C, where $A$ and $B$ are standard coefficients. This quadratic physical relationship serves as the theoretical justification for using Quadratic Least Squares (LSQ) models in this study.
...
\begin{itemize}
    \item \textbf{Setup 1: High-Precision Wheatstone Bridge with ADC.} This uses a Wheatstone bridge configuration coupled with an external 16-bit ADS1115 ADC \cite{ads1115_datasheet}. This setup allows for differential measurement, significantly reducing noise and common-mode errors.
    \item \textbf{Setup 2: Simple Voltage Divider.} This uses a simple voltage divider utilizing the Arduino's internal 10-bit ADC.
\end{itemize}
