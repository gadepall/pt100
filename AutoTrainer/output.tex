...
This script uses OpenCV\cite{opencv_library} and EasyOCR\cite{easyocr} to capture video from a webcam, identify and read text from two specific regions (voltage and temperature in this case), and log this data to a file.
...
\begin{itemize}
    \item DroidCam\cite{droidcam}: An application to use a smartphone as a high-quality webcam.
    \item OBS Studio\cite{obsstudio}: This software (installed in the computer) was used to capture the DroidCam feed and output it as an "OBS Virtual Camera," which was then selected by the Python script.
\end{itemize}
